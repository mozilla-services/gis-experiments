"""
Code taken from http://stackoverflow.com/questions/8318113
"""
import math
import numpy
...
def ecef_to_lla(ecef):
    ...
    latg = math.atan2(
        Z + e * e * (1. - f) / (1. - e * e) * a * pow(math.sin(latr), 3),
...
def lla_distance(a, b):
    """Returns the distance between two locations provided in LLA in meters.
    """
    R = 6378137
...
def find_zero_height(w):
    """Determines the normalization of a homogeneous vector in ECEF using
    primitive secant root finding. This is very sensible to a good choice
    of initial x0 and x1 and needs some improvements.
    """
    x0 = 1
    x1 = 0.5
...
def trilaterate(locs, dists):
    ...
    y = ((pow(DistA, 2) - pow(DistC, 2) + pow(i, 2) + pow(j, 2)) / (2 * j)) - \
        ((i * x) / j)
